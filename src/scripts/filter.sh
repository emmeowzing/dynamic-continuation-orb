    ...
    printf "%s" "$SH_MODULES" | awk NF | while read -r module; do
        ...
        if [ "${module_dots}" = "." ]; then
            ...
            if [ "$CIRCLE_BRANCH" = "$SH_DEFAULT_BRANCH" ]; then
                if [ "$(git diff-tree --no-commit-id --name-only -r HEAD~"$SH_SQUASH_MERGE_LOOKBEHIND" "$SH_DEFAULT_BRANCH" . | awk NF | wildmatch -c ".circleci/$SH_ROOT_CONFIG.ignore")" != "" ] || { [ "$(git diff-tree --no-commit-id --name-only -r HEAD~"$SH_SQUASH_MERGE_LOOKBEHIND" "$SH_DEFAULT_BRANCH" ".circleci/$SH_ROOT_CONFIG.yml" | awk NF)" != "" ] && [ "$SH_INCLUDE_CONFIG_CHANGES" -eq 1 ]; }; then
                    printf "%s\\n" "$module_dots" >> "$SH_MODULES_FILTERED"
                    info "including \"/$module_slashes\" corresponding workflow \".circleci/${module_dots}.yml\""
                fi
            else
                if [ "$(git diff-tree --no-commit-id --name-only -r HEAD "$SH_DEFAULT_BRANCH" . | awk NF | wildmatch -c ".circleci/$SH_ROOT_CONFIG.ignore")" != "" ] || { [ "$(git diff-tree --no-commit-id --name-only -r HEAD "$SH_DEFAULT_BRANCH" ".circleci/$SH_ROOT_CONFIG.yml" | awk NF)" != "" ] && [ "$SH_INCLUDE_CONFIG_CHANGES" -eq 1 ]; }; then
                    printf "%s\\n" "$module_dots" >> "$SH_MODULES_FILTERED"
                    info "including \"/$module_slashes\" corresponding workflow \".circleci/${module_dots}.yml\""
                fi
            fi
...
IGNORE
        ...
        if [ "$CIRCLE_BRANCH" = "$SH_DEFAULT_BRANCH" ]; then
            if [ "$(git diff-tree --no-commit-id --name-only -r HEAD~"$SH_SQUASH_MERGE_LOOKBEHIND" "$SH_DEFAULT_BRANCH" . | awk NF | wildmatch -c ".circleci/${module_dots}.ignore")" != "" ] || { [ "$(git diff-tree --no-commit-id --name-only -r HEAD~"$SH_SQUASH_MERGE_LOOKBEHIND" "$SH_DEFAULT_BRANCH" .circleci/"$module_dots".yml | awk NF)" != "" ] && [ "$SH_INCLUDE_CONFIG_CHANGES" -eq 1 ]; }; then
                printf "%s\\n" "$module_dots" >> "$SH_MODULES_FILTERED"
                info "including \"/$module_slashes\" corresponding workflow \".circleci/${module_dots}.yml\""
            fi
        else
