...
if "$SH_FORCE_ALL" || { [ -n "$SH_REPORTING_WINDOW" ] && [ "$(curl -s -X GET --url "https://circleci.com/api/v2/insights/$SH_PROJECT_TYPE/${SH_CIRCLE_ORGANIZATION}/${CIRCLE_PROJECT_REPONAME}/workflows?reporting-window=${SH_REPORTING_WINDOW}" --header "Circle-Token: $SH_CIRCLE_TOKEN" | jq -r '[ .items[].name ] | length')" -eq 0 ]; }; then
